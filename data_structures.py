...
class DictStack:
    ...
    def join(self, delimiter):
        string = ''
        for i, key in enumerate(self.__items.keys()):
            if i != len(self.__items) - 1:
                string += f'{key}: {self.__items[key]}{delimiter}'
            else:
                string += f'{key}: {self.__items[key]}'
        return string

    def index(self, item):
        for i, value in enumerate(self.__items.values()):
            if value == item:
                return i
